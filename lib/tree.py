...
Copyright (c) 2007-2008 Roc Zhou (÷‹≈Ù)

This file is part of caxes.

caxes is free software; you can redistribute it and/or modify
it under the terms of the GNU General Public License as published by
the Free Software Foundation; either version 2 of the License, or
(at your option) any later version.

caxes is distributed in the hope that it will be useful,
...
(1) Create a Tree with node value, several sub Tree attributes and key indexed
sub Tree items:
root = Tree(value, {'x' : v1, 'y' : v2}, trunk=v3, branch=v4)

(2) The sub Trees, both of attributes and key indexed items, can contain sub
Trees, too:
root = Tree(value, {'x' : v1, 'y' : Tree(v2, {'z' : v3}, br=v4)}, trunk=v5, branch=Tree(v6, {'a' : v7}, br=v8))
By this way, a whole more complex Tree can be built all at once, of course it
can also be built like this(refer to __setattr__ and __setitem__):
root = Tree(value)
root['x'] = v1
root['y'] = v2
root['y']['z'] = v3
...
The "root.branch.br1 = value" works on root.branch rather than root.branch.br1
, thus root must be a Tree instance at first.
...
'root.branch.br1', the 'root.branch' and 'root.branch.br1' will been replaced
by the latter statement.

You can also use "root.branch = Tree(value, br1=value1, br2=value2)" to create
sub nodes of 'root.branch', refer to __init__, and keep the "replacement"
problem in head.
...
To set the value of the Tree head node is not as easy as sub nodes, the latter
can make use of "setattr" directly. Thus 'set' action is very useful for this.
